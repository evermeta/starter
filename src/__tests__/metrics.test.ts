...
describe('MetricsService', () => {
    ...
    it('should record HTTP requests', async () => {
        ...
        expect(response.text).toContain('http_requests_total{method="GET",path="/test",status="200"} 1');
        expect(response.text).toContain('http_requests_total{method="POST",path="/api/data",status="201"} 1');
    });
